  ...
  pushd "$MONOREPO_NAME"
	...
	read_repositories | while read repo name folder; do
		...
		remote-branches "$name" | while read branch; do
		  if [[ "$branch" = "preview" ]]; then continue; fi;
		...
		if [[ $name = "controller" || $name = "hmi" || $name = "flask_server" || $name = "balcony" ]]; then
		  main_branch="preview"
		else
		  main_branch="master"
		fi
    ...
    git checkout -q -- .
    git clean -f -d
    git merge -q --no-commit -s ours "$name/$main_branch" --allow-unrelated-histories
    git read-tree --prefix="$folder/" "$name/$main_branch"
    git commit -q --no-verify --allow-empty -m "Merging $name $main_branch to master"
	done
...
#cat repos.txt.txt | bash tomono/tomono.sh
